...
from .util.cookie import _ExtendedCookie, _ExtendedMorsel
...
class Response(object):
    ...
    def iter_encoder(self, iter):
        for s in iter:
            if isinstance(s, unicode):
                if self.charset is None:
                    raise TypeError(
                        "You cannot set the body to a unicode value without a charset")
                yield s.encode(self.charset)
    ...
    def __call__(self, environ, start_response):
        ...
        # IVM: add iter_encoder
        return self.iter_encoder(self.app_iter)
